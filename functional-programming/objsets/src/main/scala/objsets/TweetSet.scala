...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    def compareTweets(a: Tweet, b: Tweet): Tweet = {
      if (a.retweets > b.retweets) a
      else b
    }
    if (left.isEmpty && right.isEmpty) elem
    else if (left.isEmpty && !right.isEmpty) compareTweets(right.mostRetweeted, elem)
    else if (!left.isEmpty && right.isEmpty) compareTweets(left.mostRetweeted, elem)
