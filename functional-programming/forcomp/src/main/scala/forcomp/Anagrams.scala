...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    occurrences.reverse.foldLeft(List[Occurrences](Nil)) {
      case (allOccurrences, (char, int)) =>
        allOccurrences ++ ( for {
          occurrence <- allOccurrences
          i <- 1 to int } yield (char, i) :: occurrence )
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    x.foldLeft(List[(Char, Int)]()) {
      case (acc, (char, int)) =>
        if (y.toMap.get(char).isEmpty) acc :+ (char, int)
        else {
          val frequency = int - y.toMap.get(char).get
          if (frequency >= 1) acc :+ (char, frequency)
          else acc
        }
    }
  }.sortBy(_._1)
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def getSentences(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) return List(Nil)
      for {
        combination <- combinations(occurrences)
        word <- dictionaryByOccurrences.getOrElse(combination, Nil)
        s <- getSentences(subtract(occurrences, wordOccurrences(word)))
      } yield word :: s
    }
